...
export default function CertificationCourse({ code, level, title, description, scope, sourceUrl, sourceLabel }: Props) {
  ...
  return <main className="learning-shell course-page">
    ...
    <header className="course-hero compact-course-hero"><div className="cert-mark"><span>AWS</span><strong>{code}</strong></div><div className="course-title"><p className="course-kicker">{level} certification learning path</p><h1><span className="cert-title-icon" aria-hidden="true">✓</span><span>{title}</span></h1><p>{description}</p><a className="scope-source" href={sourceUrl} target="_blank" rel="noreferrer">Official AWS in-scope services ↗</a></div><div className="course-overview"><div><strong>{scope.length}</strong><span>scope categories</span></div><div><strong>{uniqueServices}</strong><span>unique services</span></div><div><strong>{references}</strong><span>scope references</span></div></div></header>
