...
export default function CertificationCourse({ code, level, title, description, scope, sourceUrl, sourceLabel }: Props) {
  ...
  const selectCourseService = (name: string) => { const match = findGuide(name); if (!match) return; const scopeName = availableScopeServices.find((candidate) => findGuide(candidate)?.name === match.name) || match.name; setSelectedScopeName(scopeName); setImageScale(100); setVisualVisible(true); setSidebarCollapsed(false); requestAnimationFrame(() => document.getElementById("course-services")?.scrollIntoView({ behavior: "smooth", block: "start" })); };
