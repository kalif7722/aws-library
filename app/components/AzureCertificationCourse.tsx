...
export default function AzureCertificationCourse({ course }: Props) {
  ...
  return <main className="learning-shell course-page">
    ...
    <section className={`course-workspace ${sidebarCollapsed ? "sidebar-collapsed" : ""}`} id="curriculum">
      ...
      {selected && <article className="viewer course-viewer has-knowledge" style={{ "--service-accent": "#0078d4" } as CSSProperties}><div className="viewer-head clean-viewer-head"><div><h2>{selected.name}</h2><small>{selectedCategory} · {selectedEntry?.classification || "Central service page"}</small></div><div className="azure-viewer-actions"><button className="visual-toggle" onClick={() => { setVisualVisible((value) => { if (value) setExpanded(false); return !value; }); }}>{visualVisible ? "Hide visual ↑" : "Show visual ↓"}</button><a className="visual-toggle" href={`/azure-services?service=${encodeURIComponent(selected.slug)}`}>Open service library →</a></div></div>{visualVisible && (imageError ? <div className="azure-pending-card"><strong>EL10 visual pending</strong><p>The central service page is connected, but the matching R2 visual is not available at <code>{imagePath}</code>.</p></div> : <button className="image-link clean-image-link" onClick={() => setExpanded(true)} aria-label={`Open ${selected.name} visual full screen`}><img src={azureAssetUrl(imagePath)} alt={`${selected.name} Azure study visual`} onError={() => setImageError(true)} /></button>)}<AzureServiceLearningDetails serviceName={selected.name} /></article>}
    </section>
