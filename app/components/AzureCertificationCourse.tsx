...
import Az900CourseGuide from "./Az900CourseGuide";
import AzureAdvancedCourseGuide from "./AzureAdvancedCourseGuide";
...
export default function AzureCertificationCourse({ course }: Props) {
  ...
  useEffect(() => {
    const onCourseService = (event: Event) => {
      ...
      if (detail?.courseCode !== course.code || !detail.name) return;
      const entry = allServices.find((item) => item.name === detail.name);
      if (!entry) return;
      selectService(entry);
      document.getElementById("curriculum")?.scrollIntoView({ behavior: "smooth", block: "start" });
    };
    window.addEventListener("azure-course-service", onCourseService);
    return () => window.removeEventListener("azure-course-service", onCourseService);
  }, [course.code, allServices]);
  ...
  return <main className="learning-shell course-page">
    ...
    {["AZ-104", "AZ-305", "AZ-700", "AZ-400"].includes(course.code) && <AzureAdvancedCourseGuide course={course} />}
    {course.code === "AZ-900" && <Az900CourseGuide course={course} />}
    {!["AZ-104", "AZ-305", "AZ-700", "AZ-400", "AZ-900"].includes(course.code) && <AzureCourseTaskWalkthrough course={course} />}
